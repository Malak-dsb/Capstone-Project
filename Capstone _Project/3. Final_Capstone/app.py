"""🏡 Advanced House Price Predictor — Capstone Enterprise Edition"""
...
st.markdown(
    ...
    .price-box h2 { font-size: 2.5rem; margin: 0; }
...
with tab1:
    ...
    with left:
        st.markdown(
            f"""
            <div class="price-box">
                <p style="margin-bottom:0px;">Estimated Valuation</p>
                <h2>${predicted_price:,.0f}</h2>
                <p style="font-size:0.9rem; margin-top:5px;">Confidence Interval: <b>${max(0, predicted_price - mae):,.0f}</b> – <b>${predicted_price + mae:,.0f}</b></p>
            </div>
            """,
            unsafe_allow_html=True,
        )
...
with tab3:
    ...
    sqft_range = [
        int(sqft_living * factor)
        for factor in [0.8, 0.9, 1.0, 1.1, 1.2, 1.3, 1.4, 1.5]
    ]
    scenario_prices = []

    for sqft in sqft_range:
        temp_input = user_input.copy()
        temp_input["sqft_living"] = sqft
        pred = np.expm1(model.predict(temp_input)[0])
        scenario_prices.append(pred)

    scenario_df = pd.DataFrame(
        {"Sqft Living": sqft_range, "Predicted Price": scenario_prices}
    )
    ...
    fig_whatif = px.line(
        ...
        x="Sqft Living",
        y="Predicted Price",
        title="Valuation Trajectory based on Sqft Living Expansion",
        markers=True,
    )
    ...
    fig_whatif.update_traces(line_color="#1abc9c", line_width=3)
